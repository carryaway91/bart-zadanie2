...
const GalleryList: React.FC<IProps> = ({ selectGallery, setLoaded, loaded, reload, cancelReload }) => {
    ...
    const [imgs, setImgs] = useState<{path: string, image?: {path: string, fullpath: string, name: string, modified: string }, name: string, img: string }[]>([])
    const [showSelection, setShowSelection] = useState(false)
    const [galleryImgs, setGalleryImgs] = useState<any>([])
    const [finishedGallery, setFinishedGallery] = useState<any>([])
  ...
  const addPicsToGalleries = (arrOfPics: any, filteredPics: any) => {
    const newArray: any = []
     ...
     for(let i = 0; i < arrOfPics.length - 1; i++) {
      ...
      setFinishedGallery(filteredArray)
